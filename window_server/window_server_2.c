...
unsigned int WINAPI UDP_Thread(void* arg) {

	udp_thread_factor utf = *((udp_thread_factor*)arg);

	SOCKADDR_IN client_addr = utf.client_addr;
	...
	printf("Client IP : %s Port : %d\n", inet_ntoa(utf.client_addr.sin_addr), ntohs(utf.client_addr.sin_port));
	...
	sendto(utf.sock, T_int, sizeof(T_int), 0, (SOCKADDR*)&utf.client_addr, client_len);
...
int TCP_server(SOCKADDR_IN* server_addr) {
	...
	while ((new = accept(sock, (SOCKADDR*)&client_addr, &client_len)) != SOCKET_ERROR) { // 새로운 소켓으로 통신
		...
		if ((h_thread = (HANDLE)_beginthreadex(NULL, 0, Server_Socket_Thread, (void*)new, 0, (unsigned*)&thread_id)) == 0) // if thread create is failed, return 0
...
int UDP_server(SOCKADDR_IN* server_addr) {
	...
	if ((utf.sock = socket(AF_INET, SOCK_DGRAM, 0)) == INVALID_SOCKET)
		err("UDP socket()");

	printf("UDP socket() success\n");

	if (bind(utf.sock, (SOCKADDR*)server_addr, sizeof(*server_addr)) == SOCKET_ERROR /* SOCKET_ERROR == -1 */)
		err("bind()");

	printf("bind() success\n");

	while (recvfrom(utf.sock, recv_buf, recv_buflen, 0, (SOCKADDR*)&utf.client_addr, &client_len) != SOCKET_ERROR){
		if ((h_thread = (HANDLE)_beginthreadex(NULL, 0, UDP_Thread, (void*)&utf, 0, (unsigned*)&thread_id)) == 0) // if thread create is failed, return 0
			err("thread error");
		
	}

	closesocket(utf.sock); // Unlike using close in Linux, use closesocket function to close socket in Window
